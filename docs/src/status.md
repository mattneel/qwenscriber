...
| TypeScript SDK | Partial | `Qwenscriber.capabilities()`, typed errors, resumable download with digest verification, IndexedDB cache, worker channel, preprocessing, WebGPU adapter/limit probing with an upload planner, and the model/decode ABI bindings including tensor enumeration. `transcribe()` still stops with a typed error when no converted model is loaded; `stream()` has no surface |
