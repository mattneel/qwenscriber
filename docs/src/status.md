...
| Versioned C-like ABI (v1) | Partial | Exported: ABI/core version, allocator, log-mel, tokenizer/detokenizer, self-test, and the model family (begin, add_shard, finish, requirements) with decode (begin, step, tokens, end). `tools/wasm_selftest.mjs` covers the invalid-state paths and shard validation. A feature-bits query and a native C header are still missing |
...
| WASM linear-memory budget | Measured | A wasm32 instance tops out at 2 GiB of linear memory in Node, so a converted model with a 2048-position f32 key/value cache (0.87 GiB total) fits and one with 8192 positions (1.75 GiB of cache plus 426 MB of weights) does not. The converter's `--max-positions` is what selects the runnable budget |
