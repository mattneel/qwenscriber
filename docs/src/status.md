...
| Versioned C-like ABI (v1) | Partial | Feature bits report 0x1f: version, allocator, log-mel, tokenizer, self-test, model, decode. The model family (begin, add_shard, finish, set_cache_format, requirements, audio_config, tensor_count, tensor_descriptor) and decode (begin, step, tokens, end) are exported and covered by `tools/wasm_selftest.mjs` — invalid-state matrix, shard validation, requirements arithmetic, and a real decode. A native C header and the audio family are still missing |
| ABI conformance harness | Implemented | `zig build test-wasm` runs `tools/wasm_selftest.mjs` against the real module in Node |
| TypeScript SDK | Partial | `Qwenscriber.capabilities()`, typed errors, resumable download with digest verification, IndexedDB cache, worker channel, preprocessing, WebGPU adapter/limit probing with an upload planner, and the model/decode ABI bindings including tensor enumeration and the audio tower's geometry, and the GPU side of the audio tower (`gpu/{quant_layout,tower_weights,conv_stack,audio_tower}.ts`). `transcribe()` still stops with a typed error when no converted model is loaded; `stream()` has no surface |
...
| WGSL kernel suite | Experimental | 19 kernels in `gpu/shaders`: the decoder's matmuls, RMSNorm, RoPE, causal attention, gated SiLU, argmax, q8 cache quantization, q8 decode attention, and quantized row gather, plus the audio tower's LayerNorm, GELU, 3x3 stride-2 convolution, f16-weight matmul, transpose, elementwise add, and row-broadcast bias add. `node tests/gpu/harness.mjs` compares 24 cases against Zig CPU references and passes under SwiftShader on this host (worst case: RoPE `max_abs` 5.0e-4), including a `quant_hash` oracle that matches the Zig self-test bit for bit |
| WebGPU backend integration | Partial | The SDK acquires a device, reads the adapter's real limits, requests the raised `max` limits a caller declares — a device starts at the specification's 16384-byte workgroup-storage default, which the convolution kernel's 18432 staged bytes exceed, and an under-declared device answers with zeros rather than an error — plans sharded uploads, and runs `matmul_q4` end to end (`packages/qwenscriber/src/gpu/{matmul_q4,runtime,upload_plan}.ts`), exercised by `examples/browser`. The whole audio tower runs on the GPU: `node tests/gpu/tower.mjs` uploads its weights from a converted model, runs the convolution stack per chunk (six dispatches), packs the steps the valid mel frames produce, and runs eighteen pre-norm blocks plus `ln_post` (325 dispatches for a 55-step clip) in 258 ms under SwiftShader. Against the same conversion's own CPU stage dump the convolution agrees to `max_abs` 1.2e-5 and the blocks to 7.8e-6 scale relative, where `tools/reference/compare_fixtures.py` allows 2e-2; our tower output sits exactly as far from the released implementation as that dump does, so what remains is the q5 quantization both share. The decoder is not dispatched yet, so `backend: "webgpu"` still does not produce a transcript |
