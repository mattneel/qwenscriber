...
| WGSL kernel suite | Experimental | 15 kernels in `gpu/shaders`: the decoder's matmuls, RMSNorm, RoPE, causal attention, and gated SiLU, plus the audio tower's LayerNorm, GELU, 3x3 stride-2 convolution, f16-weight matmul, transpose, elementwise add, and row-broadcast bias add. `node tests/gpu/harness.mjs` compares 18 cases against Zig CPU references and passes under SwiftShader on this host (worst case: RoPE `max_abs` 5.0e-4), including a `quant_hash` oracle that matches the Zig self-test bit for bit |
