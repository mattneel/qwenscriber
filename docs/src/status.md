...
| TypeScript SDK | Partial | `Qwenscriber.capabilities()`, typed errors, resumable download with digest verification, IndexedDB cache, worker channel, preprocessing, WebGPU adapter/limit probing with an upload planner, and the model/decode ABI bindings including tensor enumeration and the audio tower's geometry, and the GPU side of the tower's first stage (`gpu/{quant_layout,tower_weights,conv_stack}.ts`). `transcribe()` still stops with a typed error when no converted model is loaded; `stream()` has no surface |
...
| WebGPU backend integration | Partial | The SDK acquires a device, reads the adapter's real limits, requests the raised `max` limits a caller declares — a device starts at the specification's 16384-byte workgroup-storage default, which the convolution kernel's 18432 staged bytes exceed, and an under-declared device answers with zeros rather than an error — plans sharded uploads, and runs `matmul_q4` end to end (`packages/qwenscriber/src/gpu/{matmul_q4,runtime,upload_plan}.ts`), exercised by `examples/browser`. The audio tower's first stage runs on the GPU: `node tests/gpu/tower.mjs` uploads the convolution and projection weights from a converted model, runs one chunk as six dispatches, and matches the same conversion's own stage dump at `max_abs` 1.2e-5 against a 1e-3 tolerance. The tower's blocks and the decoder are not dispatched yet, so `backend: "webgpu"` still does not produce a transcript |
