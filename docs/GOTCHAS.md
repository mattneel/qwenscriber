...
| `name shadows primitive 'f32'` | A declaration may not be named after a primitive type either, including an enum field: `f32 = 0` is refused. Name the variant for its meaning (`full_precision`) and parse the wire value with `std.meta.intToEnum` |
...
| `std.meta.intToEnum` does not exist | Parsing a wire integer into an enum is `std.enums.fromInt(E, value)`, which returns an optional |
